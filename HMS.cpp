...
void managePatients(vector<Patient>& patientList, vector<Doctor>& doctors, vector<Nurse>& nurses, vector<Technician>& technicians, vector<Room>& rooms) {
    ...
    int hour;
    cin >> hour;
    if (hour >= 0 && hour < HOURS_IN_DAY && selectedStaff->timetable[hour] == "Free") {
...
int main() {
  ...
  vector<Staff> staffList;
    ...
    do {
        ...
        Patient* selectedPatient = nullptr;
        vector<Patient> pastPatients; // Add this to the main scope
    ...
    Patient* selectedPatient = nullptr;
    for (auto& patient : patientList) {
        if (patient.name == id) { // Match by name
            selectedPatient = &patient;
            break;
        }
    }
    ...
    if (selectedPatient) {
        ...
        displayPatientChart(*selectedPatient); // Display patient chart
        ...
        // Add to pastPatients and remove from active list
        pastPatients.push_back(*selectedPatient);
        patientList.erase(remove(patientList.begin(), patientList.end(), *selectedPatient), patientList.end());

        cout << "Patient discharged successfully!\n";
    } else {
        cout << "Patient not found. Returning to menu...\n";
    }
    break;
}
...
case 6: {
    ...
    if (pastPatients.empty()) {
        cout << "No discharged patients available.\n";
        break;
    }

    cout << "+----------------------+-----+---------------------+-----------------------+-------------------+\n";
    cout << "| Patient ID           | Age | Reason for Visit    | Assigned Department  | Discharge Date    |\n";
    cout << "+----------------------+-----+---------------------+-----------------------+-------------------+\n";

    for (const auto& patient : pastPatients) {
        cout << "| " << setw(20) << left << patient.id
             << "| " << setw(3) << patient.age
             << "| " << setw(20) << patient.reasonForVisit
             << "| " << setw(20) << patient.department
             << "| " << setw(18) << patient.dischargeDate << "|\n";
    }
